...
if __name__ == "__main__":
    ...
    ran_top_array = np.zeros((N_zlam * N_RANDOMS, len(Rmid)))
    ran_top_im_array = np.zeros((N_zlam * N_RANDOMS, len(Rmid)))
    ran_bottom_array = np.zeros((N_zlam * N_RANDOMS, len(Rmid)))
    ran_weight_array = np.zeros((N_zlam * N_RANDOMS, len(Rmid)))
    ran_pz_bpz_array = np.zeros((N_zlam * N_RANDOMS, len(Rmid), 301))
    ran_pz_dnf_array = np.zeros((N_zlam * N_RANDOMS, len(Rmid), 301))

    for i, zlam in enumerate(tqdm(zlam_list)):
